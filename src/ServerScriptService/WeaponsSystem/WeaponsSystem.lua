...
function WeaponsSystem.setWeaponEquipped(weapon, equipped)
	...
	if WeaponsSystem.gui then
		--WeaponsSystem.camera:setEnabled(true)  -- Added to fix camera issues.
...
-- Jaxxon Jargon changed this function to work better.
			...
			if WeaponsSystem.playersOnDifferentTeams(dealer, targetPlayer) then
				if dealerHumanoid and target ~= dealerHumanoid and targetPlayer then
					...
					if targetPlayer.Character.Humanoid.Health == 0 then
						...
						ServerStorage.PlayerKilledEvent:Fire(targetPlayer, dealer)
					end
				end
			end
		end
...
local function _defaultGetTeamCallback(player)
	-- Jaxxon Jargon: modified to work properly.
